...
static int flash_mode_timeout = 150;
...
#define GPIO_FLASH_EN (GPIO98 | 0x80000000)
#define GPIO_TORCH_EN (GPIO97 | 0x80000000)


static struct hrtimer g_timeOutTimer;
static struct work_struct workTimeOut;

//#define FLASH_GPIO_FLASH_EN GPIO12
//#define GPIO_TORCH_EN GPIO13
...
int FL_Enable(void)
{
	...
	if(g_duty==1)
	{
		mt_set_gpio_out(GPIO_TORCH_EN,GPIO_OUT_ONE);
...
int FL_Init(void)
{
    ...
    if(mt_set_gpio_out(GPIO_FLASH_EN,GPIO_OUT_ZERO)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
    /*Init. to disable*/
...
/*Add by jinlin.Hu for pixi3-4.5-4G-vf torch mode. Start.*/
...
/*Add by jinlin.Hu for pixi3-4.5-4G-vf torch mode. End.*/
...
static void work_timeOutFunc(struct work_struct *data)
{
    ...
    if (flash_on) {
	FL_Enable();
    	ktime1 = ktime_set(0, flash_mode_timeout * 1000000);
    	hrtimer_start(&g_timeOutTimer, ktime1, HRTIMER_MODE_REL);
    }
}
...
static int constant_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
		...
		case FLASH_IOC_SET_TIME_OUT_TIME_MS:
			...
			//g_timeOutTimeMs=arg;
			g_timeOutTimeMs = flash_mode_timeout;
		break;


    	case FLASH_IOC_SET_DUTY :
    		printk("hujl@debug:++++FLASHLIGHT_DUTY: %d\n",(int)arg);
    		FL_dim_duty(arg);
    		break;
