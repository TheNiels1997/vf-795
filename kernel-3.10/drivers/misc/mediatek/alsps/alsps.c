...
static struct device *devinfo_Psensor;
static struct device *devinfo_Lsensor;
static int psensor_detect = 0;
static int lsensor_detect = 0;
	...
	for(i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++)
	{
	  ...
	  if(0 != alsps_init_list[i])
	  {
		...
		if(0 == err)
		{
		   ...
		   if(strcmp(alsps_init_list[i]->name,tpsensor)!=0)
		   {
			   psensor_detect = 1;
			   lsensor_detect = 1;
		   }
		 ...
		 err = alsps_init_list[tp_flag]->init();
		 if(0 == err)
		 {
		   ALSPS_LOG(" alsps real driver %s probe ok\n", alsps_init_list[tp_flag]->name);
		    break;
		 }
	  }
	}
