...
#ifdef CONFIG_MTK_SPEAKER
/* Modified by shitao.qiu@tcl.com audio driver begin */
//static int Speaker_mode = AUDIO_SPEAKER_MODE_AB;
static int Speaker_mode = AUDIO_SPEAKER_MODE_D;
/* Modified by shitao.qiu@tcl.com audio driver end */
...
#else
        mt_set_gpio_mode(GPIO99,GPIO_MODE_00);  // gpio mode
        mt_set_gpio_pull_enable(GPIO99,GPIO_PULL_ENABLE);
        mt_set_gpio_dir(GPIO99,GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO99,GPIO_OUT_ZERO); // high

        mt_set_gpio_dir(GPIO99,GPIO_DIR_OUT); // output
        mt_set_gpio_out(GPIO99,GPIO_OUT_ONE); // low
#endif
/* Modified by shitao.qiu@tcl.com audio driver end */
        msleep(SPK_WARM_UP_TIME);//PR979386 No Touch sound some times
        printk("Ext_Speaker_Amp_Change ON- \n");
    }
    else
    {
