...
extern int ps_report_interrupt_data(int); //add by xiewei@tcll for tp-psensor
...
/*=============================================================*/
#define key_back_menu	21 //add by xiewei
u8 key_back_menu_flag = 0;
...
unsigned char msg2xxx_yyyy_update_bin[94*1024] =
{
	//#include "Pixi3-4.5_byd_2238_V0XE.103-20150521.h"
	#include "PIXI3-4.5_BYD_2238_V0xE.106_20150624.h"
};
unsigned char msg2xxx_xxxx_update_bin[94*1024] =
{
	//#include "Pixi3-4.5_byd_2238_V0XE.103-20150521.h"
	#include "PIXI3-4.5_BYD_2238_V0xE.106_20150624.h"
};
        ...
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                if (g_EnableTpProximity && ((pPacket[5] == 0x80) || (pPacket[5] == 0x40)))
                {
                    ...
                    DBG("g_FaceClosingTp = %d\n", g_FaceClosingTp);
		    ps_report_interrupt_data(g_FaceClosingTp);	//add by xiewei@tcl for psensor		
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    DBG("*** %s() ***\n", __func__);
   TP_VENDOR = 0xA5; //add by xiewei@tcl for device info
...
void DrvFwCtrlHandleFingerTouch(void)
{
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
		...
		if((key_back_menu_flag == 1) || (key_back_menu == nTouchKeyCode))//add by xiewei@tcl
		{
	                input_report_key(g_InputDevice, BTN_TOUCH, 0);
               		input_report_key(g_InputDevice, KEY_BACK, 0);	
		        input_report_key(g_InputDevice, KEY_MENU, 0);	
			key_back_menu_flag = 0;	 
		}
		else
		{
	                input_report_key(g_InputDevice, BTN_TOUCH, 0);
        ...
        else //touch on screen
        {
	        ...
	        else if (tInfo.nTouchKeyCode == 3)//for TOUCH_KEY_MENU+TOUCH_KEY_BACK xiewei
	        {
			nTouchKeyCode = 	key_back_menu;
		}
		    ...
		    if (key_back_menu == nTouchKeyCode)//add by xiewei@tcl
		    {
			    DBG("screen pinning back and menu key report\n");
			    input_report_key(g_InputDevice, BTN_TOUCH, 1);
	                    input_report_key(g_InputDevice, KEY_BACK, 1);
			    input_report_key(g_InputDevice, KEY_MENU, 1);
			    key_back_menu_flag = 1;
