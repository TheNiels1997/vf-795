...
static void _DSI_INTERNAL_IRQ_Handler(DISP_MODULE_ENUM module, unsigned int param)
{
	int i = 0;
	 ...
	 printk("\tAAAAAA, param=%d  ", param);
	 
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
	    	//status = DSI_REG[i]->DSI_INTSTA;
		printk("\tAAAAAA,DSI_REG[%d]->DSI_INTSTA=0x%x ", i, INREG32(&DSI_REG[i]->DSI_INTSTA) );
...
{
	printk("\tAAAAAA, lcm_set_reset_pin ");
...
//		if(CMD_MODE == dsi_config->mode)
		if(dsi_config->mode == CMD_MODE ||( (dsi_config->switch_mode_enable == 1) && (dsi_config->switch_mode == CMD_MODE))) 
			DSI_OUTREGBIT(cmdq, DSI_INT_ENABLE_REG,DSI_REG[i]->DSI_INTEN,TE_RDY,0);//modify by llf for 1 to 0
	}
