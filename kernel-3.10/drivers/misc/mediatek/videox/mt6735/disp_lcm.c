...
static disp_lcm_handle *explcm = NULL;
...
{
	char ictype[]="pixi45_ili9806e_fwvga_dsi_tdt";
	char moduletype[]="TDT";
	char batch[]="SUP1";
	char remark[]="0 x 0";
	char na[]="NA:NA:NA:0";
	...
	//memcpy(plcm,explcm,sizeof(disp_lcm_handle));
	//lcm_id = lcm_driver_list[plcm->index]->compare_id();

	if(lcm_id==0){//id == 3 is TDT 1 is BYD
		if(plcm) 
			kfree(plcm);
		return sprintf(buf,"%s\n",na);
	}
	if(strcmp(plcm->drv->name,ictype)==0)// 0 mean eqaul;
	{
		char str[]="ILI9806E";
		char strid[]="BYD";
		sprintf(ictype,"%s",str);
		if(lcm_id==1){//id == 3 is TDT
			sprintf(moduletype,"%s",strid);
		}
		sprintf(remark,"%dx%d",plcm->params->width, plcm->params->height);
		printk("[LLF]  ictype :%s ------remark :%s\n",ictype,remark);
		
	}else{
		char str1[]="OTM8019A";
		char str2[]="SUPX";
		sprintf(ictype,"%s",str1);
		sprintf(moduletype,"%s",str1);
		sprintf(batch,"%s",str2);
		printk("[LLF]ictype :%s-------moduletype:%s ----------batch : %s\n",ictype,moduletype,batch);
		sprintf(remark,"%dx%d",plcm->params->width, plcm->params->height);
	}

	if(plcm) 
		kfree(plcm);
	return sprintf(buf,"%s:%s:%s:%s\n",ictype,moduletype,batch,remark);
}
...
disp_lcm_handle* disp_lcm_probe(char* plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	if((lcm_id == LCM_INTERFACE_NOTDEFINED) || lcm_id == plcm->lcm_if_id)
	{
		...
		explcm = kzalloc(sizeof(uint8_t*) *sizeof(disp_lcm_handle), GFP_KERNEL);
		memcpy(explcm,plcm,sizeof(disp_lcm_handle));
