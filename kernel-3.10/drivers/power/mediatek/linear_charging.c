...
CHR_CURRENT_ENUM	g_temp_CC_value_limit = CHARGE_CURRENT_650_00_MA;	//max charging current for pixi3-45 vf
...
kal_uint32 set_bat_charging_current_limit(int current_limit)
{
	...
	if (current_limit != -1) {
		...
		if(g_temp_CC_value > CHARGE_CURRENT_650_00_MA)
			g_temp_CC_value_limit = CHARGE_CURRENT_650_00_MA;
		else
			g_temp_CC_value_limit = g_temp_CC_value;

	} else {
		g_temp_CC_value_limit = CHARGE_CURRENT_650_00_MA; //650mA for pixi3-45 max charging current
...
/*charging temperature ploicy add by liyue yueli@tcl.com*/
		...
		if(((BMT_status.temperature < CHARGE_TEMPERATURE_STEP1)&&
            	...
            	{
			g_temp_CC_value = min(g_temp_CC_value, CHARGE_CURRENT_650_00_MA);
...
extern kal_uint32 mmitest_charging_enable; 
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA) {
			...
			printk("feichen g_temp_CC_value = %d\n",g_temp_CC_value);
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)
...
/*charging temperature ploicy add by liyue yueli@tcl.com*/
	...
	if( mmitest_charging_enable == KAL_FALSE )
		charging_enable = KAL_FALSE;
	
	battery_log(BAT_LOG_CRTI, "[BATTERY][MMITEST] pchr_turn_on_charging(), enable =%d \r\n",
			    charging_enable);
#endif
