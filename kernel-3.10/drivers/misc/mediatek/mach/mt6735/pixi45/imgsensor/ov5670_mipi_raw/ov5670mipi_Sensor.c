...
#ifdef OV5670_OTP_CALI

int rg_h,bg_h,rg_bg_l,g_ave1;
int golden_rg_h, golden_bg_h, golden_rg_bg_l, golden_g_ave;
...
static int ov5670_read_otp(void)
{
	...
	if((awb_flag&0xc0)==0x40)
	{
		...
		golden_rg_h = read_cmos_sensor(0x702C);
		golden_bg_h = read_cmos_sensor(0x702D);
		golden_g_ave = read_cmos_sensor(0x702E);
		golden_rg_bg_l = read_cmos_sensor(0x702F);
	}
	else if((awb_flag&0x30)==0x10)
	{
		...
		golden_rg_h = read_cmos_sensor(0x7035);
		golden_bg_h = read_cmos_sensor(0x7036);
		golden_g_ave = read_cmos_sensor(0x7037);
		golden_rg_bg_l = read_cmos_sensor(0x7038);
	}	
	...
	{	
		...
		golden_rg_h = read_cmos_sensor(0x703E);
		golden_bg_h = read_cmos_sensor(0x703F);
		golden_g_ave = read_cmos_sensor(0x7040);
		golden_rg_bg_l = read_cmos_sensor(0x7041);
	}
        //printk("TCL :awb_flag=%x, %x ,%x ,%x ,%x\n",awb_flag,rg_h,bg_h,rg_bg_l,g_ave1);
        printk("[ov5670 OTP]:rg_h=%x, bg_h = %x ,rg_bg_l = %x ,g_ave1 = %x\n",rg_h,bg_h,rg_bg_l,g_ave1);
        printk("[ov5670 OTP]:golden_rg_h = %x, golden_bg_h = %x, golden_g_ave = %x, golden_rg_bg_l = %x\n", golden_rg_h, golden_bg_h, golden_g_ave, golden_rg_bg_l);
...
static int ov5670_write_otp(void)
{
	int rg, bg , Golden_rg, Golden_bg, R_gain, B_gain, G_gain, Base_gain;

	if ((rg_h < 0) || (bg_h < 0) || (rg_bg_l < 0) || (golden_rg_h < 0) || (golden_bg_h < 0) || (golden_rg_bg_l < 0)) {
		printk("OV5670 OTP data is invalid, stop write OTP.\n");
		return -1;
	}

	rg = (rg_h << 2) | (rg_bg_l >> 6);
	bg = (bg_h << 2) | ((rg_bg_l >> 4) & 0x03);
	Golden_rg = (golden_rg_h << 2) | (golden_rg_bg_l >> 6);
	Golden_bg = (golden_bg_h << 2) | ((golden_rg_bg_l >> 4) & 0x03);
	R_gain = (Golden_rg * 1000) / rg;
	B_gain = (Golden_bg * 1000) / bg;
	G_gain = 1000;	

	printk("[ov5670 OTP]:rg = %x, bg = %x, Golden_rg = %x, Golden_bg = %x\n", rg, bg, Golden_rg, Golden_bg);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			//module_id = ov5670_read_otp();
			//LOG_INF("get_imgsensor_id  module id: %x\n", module_id);
			//if ((*sensor_id == imgsensor_info.sensor_id) && (module_id != 0x0c)) {	
