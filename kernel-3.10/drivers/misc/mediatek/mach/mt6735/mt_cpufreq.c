...
static struct pmic_wrap_setting pw = {
    ...
    .set[PMIC_WRAP_PHASE_DEEPIDLE] = {
        ...
        ._[IDX_DI_VSRAM_NORMAL]     = { MT6328_PMIC_VSRAM_VOSEL_OFFSET_ADDR, _BITS_(15 : 0,  0x10),  },  // will get actual value @ table init
        ...
        ._[IDX_DI_VSRAM_SLEEP_FOR_TURBO]  = { MT6328_PMIC_RG_VSRAM_VOSEL_ADDR, VOLT_TO_PMIC_VAL(95000),  },
        ._[IDX_DI_VSRAM_NORMAL_FOR_TURBO] = { MT6328_PMIC_RG_VSRAM_VOSEL_ADDR, VOLT_TO_PMIC_VAL(125000), },
#else   // 6735(M)
...
void mt_cpufreq_set_power_limit_by_pbm(unsigned int limited_power)
{
    ...
    if (p->limited_power_by_thermal != 0 && limited_power > p->limited_power_by_thermal) {
        cpufreq_info("@%s: power is limited by thermal(%d), ignore budget from PBM(%d)...\n",
            __func__, p->limited_power_by_thermal, limited_power);
        return;
    }

    if (limited_power == p->limited_power_by_pbm) {
        cpufreq_info("@%s: limited_power(%d mW) not changed, skip it!\n", __func__, limited_power);
        return;
    }
    ...
    cpufreq_info("@%s: limited power from PBM = %d\n", __func__, p->limited_power_by_pbm);
...
static int __cpuinit _mt_cpufreq_cpu_CB(struct notifier_block *nfb, unsigned long action, void *hcpu)
{
    ...
    if (dev) {
        ...
        if ((p->cpu_level == CPU_LEVEL_1 && PMIC_5A_THRO_MAX_CPU_CORE_NUM == online_cpus)
#ifdef CONFIG_CPU_DVFS_TURBO_MODE
