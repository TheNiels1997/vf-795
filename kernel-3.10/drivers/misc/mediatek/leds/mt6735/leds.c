...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			pmic_set_register_value(PMIC_ISINK_CH0_MODE,ISINK_BREATH_MODE);	//ISINK0 channel mode
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	*/	
	...
	switch (cust->mode) {
		...
		LEDS_DEBUG("LCM brightness mapping value:%ld\n",((long)((level*CONFIG_LIGHTNESS_MAPPING_VALUE)/255)));
		return ((cust_brightness_set) (cust->data)) ( (long)(level*CONFIG_LIGHTNESS_MAPPING_VALUE/255), bl_div_hal);

		case MT65XX_LED_MODE_CUST_BLS_PWM:
		if (strcmp(cust->name, "lcd-backlight") == 0) {
				bl_brightness_hal = level;
			}
		LEDS_DEBUG("BLS brightness mapping value:%ld\n",((long)((level*CONFIG_LIGHTNESS_MAPPING_VALUE)/255)));
