...
unsigned char TP_VENDOR = 0;
unsigned char TP_FW_VER = 0;

static struct device *devinfo_ctp;
...
/* touch panel probe */
static ssize_t ctp_devinfo_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
       unsigned char *strid=NULL;
       printk("*** The tp vendor  = %x***\n", TP_VENDOR);
       switch(TP_VENDOR){
               case 0xe0:
                       strid="FT3407:TDT:SUP1:";
                       break;
               case 0x53:
                       strid="FT3407:MUTTO:SUP1:";
                       break;
               case 0x87:
                       strid="FT3407:LECTRON:SUP1:";
                       break;
               case 0x85:
                       strid="FT3407:JUNDA:SUP1:";
                       break;
               case 0x5a:
                       strid="FT3407:TRULY:SUP1:";
                       break;
               case 0x80:
                       strid="FT3407:EARCH:SUP1:";
                       break;
		case 0xA5:
			strid="MSG2238:BYD:SUP1:";
			break;
               default:
                       strid="NA:NA:NA:";
                       break;
       }
       return sprintf(buf, "%sV%x\n", strid, TP_FW_VER);
}

static DEVICE_ATTR(ctp, 0444, ctp_devinfo_show, NULL);
...
#endif
	devinfo_ctp = get_deviceinfo_dev();
	if (device_create_file(devinfo_ctp, &dev_attr_ctp) < 0)
